...
def setup_db():
    ...
    try:
        c.execute("CREATE USER '{0}'@'localhost' IDENTIFIED BY '{1}';".format(DB_USER_USER, DB_USER_PASS))
        c.execute("CREATE USER '{0}'@'%' IDENTIFIED BY '{1}';".format(DB_USER_USER, DB_USER_PASS))
    except:
        pass
    c.execute("flush privileges;")
    c.execute("GRANT ALL on *.* to '{0}'@'localhost';".format(DB_USER_USER))
    c.execute("GRANT ALL on *.* to '{0}'@'%';".format(DB_USER_USER))
    c.execute("flush privileges;")
    myDB.commit()
    myDB.close()
...
def input_new_order(payload, src, dst, weight, customer_name, lock):
    with lock:
        myDB = MySQLdb.connect(host='localhost', port=3306, user=DB_USER_USER, passwd=DB_USER_PASS, db='trucking')
        cursor = myDB.cursor()
        cursor.execute("INSERT INTO payloads (payload, src, dst, weight, customer_name) VALUES ('{0}', '{1}', '{2}', '{3}', '{4}')".format(payload, src, dst, weight, customer_name))
        myDB.commit()
...
def assign_payload(truck_id, lock):
    ...
    with lock:
        ...
        if not cur_num:
            return None
        cursor.execute("UPDATE payloads SET truck_id='{0}', assigned_time='{1}' WHERE id={2}".format(truck_id, cur_time, cur_num[0]))
        myDB.commit()
        return cur_num
...
def mark_complete(payload_id, lock):
    ...
    with lock:
        myDB = MySQLdb.connect(host='localhost', port=3306, user=DB_USER_USER, passwd=DB_USER_PASS, db='trucking')
        cursor = myDB.cursor()
        cursor.execute("UPDATE payloads SET delivered_time = '{0}' WHERE id={1};".format(cur_time,payload_id))
        myDB.commit()
